...
from vector_store import search,load_vector_store
from config import  VECTOR_DB_PATH,TOP_K, DEEPSEEK_API_KEY,OPENWEATHER_API_KEY
...
def ask_knowledge_base(query: str) -> str:
    ...
    if not DEEPSEEK_API_KEY:
        return "错误: 未配置 DEEPSEEK_API_KEY，请在 .env 文件中设置。"
    try:
        return _agent.chat(query)
...
def check_kb_status() -> str:
    ...
    lines = []
    lines.append(f"DeepSeek API Key: {'已配置' if DEEPSEEK_API_KEY else '未配置'}")
